...
export function StoryBridgesModal() {
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const showEvent = () => setIsOpen(true);
    window.addEventListener('show-story-bridges', showEvent);
    return () => window.removeEventListener('show-story-bridges', showEvent);
  }, []);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-hidden">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-0 m-0 bg-white relative w-full h-full -webkit-overflow-scrolling-touch">
              <iframe 
                src="https://docs.google.com/forms/d/e/1FAIpQLSfXOyoqgQdUw90UCYuzd_cnHiNYwp5YJJQZ4emsRMLgndpBwA/viewform?embedded=true" 
                className="w-full h-full min-h-[1603px] sm:min-h-[1400px]"
                frameBorder="0" 
                marginHeight={0} 
                marginWidth={0}
                title="Lumina StoryBridges Request Form"
              >
                Loading…
              </iframe>
            </div>
