...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  if (variant === 'hover') {
    ...
    return (
      <>
        ...
        <button className={hoverButtonFinalClassName} style={style} onClick={handleButtonClick} disabled={disabled}>
  ...
  if (variant === 'ghost') {
    ...
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <button className={ghostButtonFinalClassName} style={style} onClick={handleButtonClick} disabled={disabled}>
          <span className="relative z-10 pointer-events-none">{children}</span>
          {jsRippleElements}
        </button>
      </>
  ...
  // Default variant
  const baseClasses = "relative border-none overflow-hidden isolate transition-all duration-200 cursor-pointer px-4 py-2 bg-[#C9A84C] hover:opacity-90 text-[#101B38] rounded-lg";
  const disabledClasses = disabled ? "opacity-50 cursor-not-allowed" : "";
  const buttonClasses = `${baseClasses} ${disabledClasses} ${className}`;
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
      <button className={buttonClasses} style={style} onClick={handleButtonClick} disabled={disabled}>
